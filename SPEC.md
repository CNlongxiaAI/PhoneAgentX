...
**PhoneAgentX** 是 OpenClaw 的手机端节点运行时。它让 AI 能够：
...
- **OpenClaw Node** — 作为官方节点接入，AI 通过 nodes.invoke 控制手机
...
PhoneAgentX Node Host
    ↓ HTTP (:18790)
PhoneAgentX Bridge Server (Android)
    ↓ Socket (:28200)
设备控制层
    ↓
手机硬件（触摸/滑动/截图/UI树）
...
```javascript
// PhoneAgentX Node Host 核心逻辑
class PhoneAgentXNodeHost {
    async connect() {
        // 1. Ed25519 设备签名
...
```
phoneagentx/
├── phoneagentx-node-host.js     # Node Host 主脚本（连接 Gateway）
...
- [x] Phase 1: PhoneAgentX Node Host（连接 OpenClaw Gateway）
